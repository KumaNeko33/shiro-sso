...
public class UpmsSystemController extends BaseController {
	...
	public Object list(
		...
		List<TblSystem> rows = upmsSystemService.selectByExampleForOffsetPage(upmsSystemExample, offset, limit);
	...
	@ResponseBody
	public Object create(TblSystem upmsSystem) {
		...
		upmsSystem.setCreateDateTime(new Date(time));
	...
	public String update(@PathVariable("id") int id, ModelMap modelMap) {
		TblSystem system = upmsSystemService.selectByPrimaryKey(id);
	...
	@ResponseBody
	public Object update(@PathVariable("id") int id, TblSystem upmsSystem) {
		ComplexResult result = FluentValidator.checkAll()
				.on(upmsSystem.getTitle(), new LengthValidator(1, 20, "标题"))
				.on(upmsSystem.getName(), new LengthValidator(1, 30, "名称"))
				.doValidate()
				.result(ResultCollectors.toComplex());
		if (!result.isSuccess()) {
			return new UpmsResult(UpmsResultConstant.INVALID_LENGTH, result.getErrors());
