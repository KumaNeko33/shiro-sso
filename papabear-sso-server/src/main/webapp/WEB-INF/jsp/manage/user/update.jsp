...
<div id="updateDialog" class="crudDialog">
	<form id="updateForm" method="post">
		...
		<div class="form-group">
			...
			<input id="nickName" type="text" class="form-control" name="nickName" maxlength="20" value="${user.nickName}">
		</div>
		<div class="row">
			<div class="col-lg-8 form-group">
				<label for="avatarImg">头像</label>
				<input id="avatarImg" type="text" class="form-control" name="avatarImg" maxlength="150" value="${user.avatarImg}">
			</div>
			<div class="col-lg-4">
		...
		<div class="radio">
			...
			<div class="radio radio-inline">
				<input id="locked_1" type="radio" name="status" value="-2" <c:if test="${user.status==-2}">checked</c:if>>
				<label for="locked_1">锁定 </label>
			</div>
			<div class="radio radio-inline">
				<input id="locked_2" type="radio" name="status" value="0" <c:if test="${user.status==0}">checked</c:if>>
				<label for="locked_2">未激活 </label>
			</div>
			<div class="radio radio-inline">
				<input id="locked_3" type="radio" name="status" value="-1" <c:if test="${user.status==-1}">checked</c:if>>
				<label for="locked_3">删除 </label>
			</div>
		</div>
...
function initUploader() {
	...
	var uploader = WebUploader.create({
		// swf文件路径
		swf: '${basePath}/resources/papabear-admin/plugins/webuploader-0.1.5/Uploader.swf',
		// 文件接收服务端
		method: 'POST',
		// 选择文件的按钮。可选。
	...
	uploader.on( 'fileQueued', function(file) {
		$.ajax({
			url: '${PAPABEAR_OSS_ALIYUN_OSS_POLICY}',
			type: 'GET',
			dataType: 'jsonp',
			jsonp: 'callback',
